...
--rejestry
signal KAWA_ile : std_logic_vector(7 downto 0); --ile dostepnych zasobników
...
--pomocnicze linie do wykrywania zbocza sygnalow sterujacych automatem
...
-- wpisywanie wartoœci do rejestrów programowalnych
    ...
    if reset = '1' then --aktywny reset==1
       ...
       if (adres = KAWA_ile_adr and nr_w = '1') then
        ...
        --end if;
            ...
            KAWA_ile <= KAWA_ile -  moc; --zmniejszenie iloœci zasobników
            elsif napoj = "10" then
...
-- na liscie wrazliwosci wymienione sa WSZYSTKIE wejscia dla tej logiki
    begin
        STAN_nast <= STAN;    -- stan biezacy "kreci sie w kolko" gdy nie ma zmian stanu       
        case STAN is
            when "000" => --RESET
                    STAN_nast <= "001";
            ...
            when "011" => --MOC
                if (OK_edge='1' and user_break='0') then
                    STAN_nast <= "010"; --przejdz do PLATNOSCI
                elsif user_break='1' then
...
placenie: process(moneta_in, moneta_in_edge, reset, STAN) --obsluga placenia - licznik wrzucanych monet
begin
    if reset='1' then
    MONETY_ile <= (others => '0');
...
reszta: process(reset, STAN) --obsluga placenia - licznik wrzucanych monet
begin
    if reset='1' then
    reszta_out <= '0';
        ...
        if STAN="110" then --stan PRZYGOTOWANIE napoju
            ...
            if moc_tmp < moc then
                if podajnik_tmp='0' then
                    podajnik_trig <= '1'; --wyjœcie aktywowane tyle razy, ile wybrana moc napoju
                    podajnik_tmp <= '1';
                else
                    podajnik_trig <= '0';
                    moc_tmp <= (moc_tmp+1);
...
OK_edge_finder: process (reset, clk) --wykrywanie zbocza sygnalu user_OK
begin
    if reset = '1' then
        OK_a <= '0';
        OK_aa <= '0';
...
sel_edge_finder: process (reset, clk) --wykrywanie zbocza sygnalu user_sel
begin
    if reset = '1' then
        sel_a <= '0';
        sel_aa <= '0';
...
moneta_in_edge_finder: process (reset, clk) --wykrywanie zbocza sygnalu user_sel
begin
    if reset = '1' then
        moneta_in_a <= '0';
        moneta_in_aa <= '0';
